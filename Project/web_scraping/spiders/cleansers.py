...
class CleansersSpider(scrapy.Spider):
    name = 'cleansers'
    ...
    def start_requests(self):
        yield SeleniumRequest(
            url='https://www.sephora.com/shop/cleanser?currentPage=1',
            wait_time=3,
            screenshot=True,
            callback=self.parse
            )
    ...
    def parse(self, response):
        ...
        try:
            ...
            driver.save_screenshot('before_next.png')
            sleep(4)
            next_button.click()
            sleep(7)
        ...
        #Following each product link for next parse
    #     for link in links:
    #         yield response.follow_all(
    #             url=link variable, 
    #             callback=self.parse_product, 
    #             meta={'brand': brand, 'name': name, 'relative_link': relative_link}
    #             )

    # def parse_product(self, response):
    #     name=response.request.meta['name']
    #     brand=response.request.meta['name']
